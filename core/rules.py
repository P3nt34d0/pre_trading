...
def enquadramento_cvm(df: pd.DataFrame, ordem: Ordem, limites_regra) -> RegraResultado:
    ...
    if entry in (None, "~", "NA", "N/A"):
        return RegraResultado("enquadramento_cvm", True, 0.0, 0.0, 0.0, f"Enquadramento CVM: Fundo {ordem.fundo} isento de observância.")
    ...
    if df_f.empty or not _col(df, "valor"):
        return RegraResultado("enquadramento_cvm", False, 0.0, 0.0, limite, "Enquadramento CVM: Sem linhas/coluna 'valor' para o fundo.")
    ...
    return RegraResultado(
        ...
        limite=limite,
        mensagem=f"Enquadramento CVM: {perc_proposto:.2%} do PL (mín {limite:.2%})."
    )
